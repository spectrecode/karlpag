...
$arrjson['idcat']   	= $objfunc->verificarDataPost('idcat');

if ($arrjson['idcat'] == 1){
	$arrjson['objetivo']   	= $objfunc->verificarDataPost('objetivo');
	$tipo_mensaje = "Objetivo";
}
else{
	$arrjson['objetivo']   = $objfunc->verificarDataPost('beneficio');
	$tipo_mensaje = "Beneficio";
}
...
if ($arrjson['codigo'] > 0){
	...
	//***Agregamos Galeria
	$objnot 			 	= new Reconocimiento();
	$arrjson['expcod'] 		= $objnot->AgregarObjetivos($arrjson);	
}else{
...
if ($bool_sql){	
	$mensaje = "Se agrego ".$tipo_mensaje." correctamente";
